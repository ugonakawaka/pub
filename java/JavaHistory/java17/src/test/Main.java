...
public class Main {
	...
	static class NumberTheory {
		...
		/**
		 * Modular Arithmetic: 1. (a+b)%c=(a%c+b%c)%c 2. (a*b)%c=(a%c*b%c)%c 3.
		 * (a-b)%c=(a%c-b%c+c)%c 4. (a/b)%c=(a%c*(b^-1)%c)%c -- (b^-1 is multiplicative
		 * modulo inverse)
		 */
		// Modular Addition
		...
		public int modularExponentiation(int x, int n, int MOD) {
			if (n == 0)
				...
				return modularMultiplication(x,
						modularExponentiation(modularMultiplication(x, x, MOD), (n - 1) / 2, MOD), MOD);
		}
		...
		/**
		 * Catlan number: - Time complexity: O(n*n) - Auxiliary space: O(n)
		 *
		 * NOTE: Time complexity could be reduced to O(n) but it is possible if and only
		 * if n is small or else there is a chance of getting an overflow. To decrease
		 * the time complexity to O(n) just remember nCr=nCn-r
		 */
		...
		/**
		 * Greatest Common Divisor(GCD) - It is also known as Highest Common Factor(HCF)
		 * - Time complexity: log(min(a,b)) - Auxiliary Space: O(1)
		 */
		...
		/**
		 * Extended Euclid's Algorithm: - ax+by=gcd(a,b) - Time complexity: -
		 */

		/**
		 * Least Common Multiple(LCM): - Time complexity: log(min(a,b)) - Auxiliary
		 * Space: O(1)
		 */
